...
from utils import if_yt_video_exists, get_subtitles
...
async def generate_summary(video_id: str):
    ...
    # extracting subtitles of requested video id
    resp = get_subtitles(video_id=video_id)
    if resp.get("success") == False: 
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=resp.get("message")
        )
